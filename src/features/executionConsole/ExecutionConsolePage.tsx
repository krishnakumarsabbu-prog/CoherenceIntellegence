...
import JupyterConsole from "./components/JupyterConsole";
import CoherenceBrainPanel from "./components/CoherenceBrainPanel";
import {
  MetricCard,
  MetricCardSkeleton,
  ExecutionChart,
...
export default function ExecutionConsolePage() {
  ...
  return (
    ...
    >
      ...
      <ConsoleToggleBar open={consoleOpen} onToggle={() => setConsoleOpen(!consoleOpen)} />
...
function ConsoleToggleBar({ open, onToggle }: { open: boolean; onToggle: () => void }) {
  const status = useExecutionStore((s) => s.status);
  const logs = useExecutionStore((s) => s.logs);
  const isBusy = status === "running" || status === "starting";

  return (
    <div className="flex items-center justify-between px-4 py-2.5 bg-slate-100 ring-1 ring-slate-200 rounded-xl">
      <div className="flex items-center gap-2 text-xs font-semibold text-slate-600">
        <span className={`w-2 h-2 rounded-full ${isBusy ? "bg-emerald-500 animate-pulse" : "bg-slate-400"}`} />
        <span>Jupyter Notebook Console</span>
        {logs.length > 0 && (
          <span className="text-[10px] font-mono text-slate-500">- {logs.length} cells executed</span>
        )}
      </div>
      <button
        onClick={onToggle}
        className="px-3 py-1.5 rounded-lg text-xs font-bold bg-blue-600 text-white hover:bg-blue-700 transition-all shadow-sm flex items-center gap-1.5"
      >
